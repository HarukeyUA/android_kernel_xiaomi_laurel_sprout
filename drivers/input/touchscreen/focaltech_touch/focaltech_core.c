...
*  Return:
*****************************************************************************/
extern void fts_fod_recovery(struct fts_ts_data *ts_data);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if(ctp_is_probe == 0)
	{
		fts_write_reg(0x8B, b_usb_plugin);
		ctp_is_probe =1;
	}
#endif
#if defined(CONFIG_FB)
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
	...
	if (msm_drm_unregister_client(&ts_data->fb_notif))
		FTS_ERROR("Error occurred while unregistering msm_drm_notifier.");
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int fts_ts_resume(struct device *dev)
{
	...
	mutex_lock(&fts_data->report_mutex);
#ifdef FTS_FOD_EN
	FTS_DEBUG("FTS:%s finger_in_fod:%d\n", __func__, ts_data->finger_in_fod);
	if (!ts_data->finger_in_fod)
#endif
		fts_release_all_finger();
	mutex_unlock(&fts_data->report_mutex);
	...
	if (!ts_data->ic_info.is_incell) {
		...
		if(tid->fod_status || tid->aod_status)
			value = 2;
...
{
	unsigned char regul_val;
	int ret;

	switch (grip_value) {
	case 0:
		ret = fts_write_reg(FTS_REG_GRIP_DISABLE, 1);
		if (ret < 0) {
			FTS_ERROR("grip_area set i2c translate fail");
			return -EIO;
		}
		return 0;
	case 1:
	...
	ret = fts_write_reg(FTS_REG_GRIP_MODE, regul_val);
	if (ret < 0) {
		FTS_ERROR("grip_area set i2c translate fail");
		return -EIO;
	}
			   ...
			   unsigned int *minor)
{
	...
	ret = fts_read_reg(FTS_REG_FW_VER, &dump);
	if (ret < 0) {
		FTS_ERROR("read firmware ID error");
		return -1;
	}

	*minor = (unsigned int)dump;
	return 0;
}
